...
- **Chat with Buddy** — AI assistant that knows your identity, values, and habits
- **Daily Journal** — Write daily logs with mood tracking, stored as vector embeddings
- **Dashboard** — Habit tracker, to-do list (with date picker), and AI-generated daily summary
- **RAG Memory** — All conversations, journals, and todos are stored in ChromaDB and used as context for smarter responses
- **Smart Filtering** — Buddy understands time references (เมื่อวาน, last week, 3 วันก่อน) and filters memory by date/type
- **Streaming Responses** — Real-time streamed chat with markdown rendering
...
This starts ChromaDB with token authentication on port 8000.
...
├── app/
│   ├── api/
│   │   ├── chat/route.ts       # Streaming chat API with RAG + persona
...
│   ├── DashboardView.tsx        # Dashboard with habits, todos, summary
│   ├── JournalView.tsx          # Journal entry form + history
...
└── lib/
    ...
    ├── memory.ts                # ChromaDB client (add, query, update logs)
...
1. All inputs (chat, journal, todos) are embedded via `nomic-embed-text` and stored in ChromaDB
